...
def server(bot, channel, message, bquit)
  sleep(5)
  puts "-------- SENDING MESSAGE"
  bot.handlers.dispatch(:monitor_msg, nil, message, channel)
  sleep(5)
  if bquit
    bot.quit()
    exit
...
messages = Array.new
lines = eat("http://beta.modd.live/api/bot_boost.php", :timeout =>40).split("\n")
...
if lines.length > 0
  ...
  whisper_array.each do |channel, message|
    quitbot = false
    Thread.new { server(bot, "#" + channel, message, quitbot) }
    #sleep(3)
  end
